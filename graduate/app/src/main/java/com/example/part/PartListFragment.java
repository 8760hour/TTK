...
public class PartListFragment extends Fragment {
        ...
        responseListener = new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                ...
                try {
                    ...
                    //Toast.makeText(PartListFragment.this.getActivity(), arr.size() + "개 널이었음", Toast.LENGTH_LONG).show();
    ...
    public void loadData(){
        if(!end) {
            ...
            //Toast.makeText(this.getActivity(), url, Toast.LENGTH_LONG).show();
        ...
        clv.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
                ...
                if(!clv.canScrollVertically(1)) {
                    if (!end) {
                        if(arr.size() % limit < limit && arr.size() % limit >= 1) {
                            end = true;
                            return;
                        }
                        if (slide) {
